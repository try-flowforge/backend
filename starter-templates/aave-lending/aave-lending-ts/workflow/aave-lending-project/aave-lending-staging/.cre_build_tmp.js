...
var tokenInfoSchema = exports_external.object({
  address: exports_external.string(),
  symbol: exports_external.string().optional(),
  decimals: exports_external.number().optional(),
  aTokenAddress: exports_external.string().optional()
});
var inputConfigSchema = exports_external.object({
  ...
  interestRateMode: exports_external.enum(["STABLE", "VARIABLE"]).optional(),
  onBehalfOf: exports_external.string().optional(),
  referralCode: exports_external.number().optional()
});
var configSchema = exports_external.object({
  chain: exports_external.string(),
  chainSelectorName: exports_external.string().optional(),
  provider: exports_external.literal("AAVE"),
  aaveReceiverAddress: exports_external.string(),
  poolAddress: exports_external.string().optional(),
  gasLimit: exports_external.string(),
  inputConfig: inputConfigSchema,
  simulateFirst: exports_external.boolean().optional()
});
var OP_SUPPLY = 0;
var OP_WITHDRAW = 1;
var OP_BORROW = 2;
var OP_REPAY = 3;
var INTEREST_RATE_STABLE = 1;
var INTEREST_RATE_VARIABLE = 2;
function deepMerge(base, overrides) {
  ...
  for (const key of Object.keys(overrides)) {
    ...
    if (val === undefined || val === null || val === "")
      continue;
    if (typeof val === "object" && !Array.isArray(val) && typeof base[key] === "object" && !Array.isArray(base[key])) {
...
function resolveChainDefaults(config) {
  ...
  if (!chainSelectorName) {
    throw new Error(`No chainSelectorName configured and chain "${config.chain}" is not in the registry`);
  }
...
function getEvmClient(chainSelectorName, testnet) {
...
function getInterestRateMode(mode) {
  if (mode === "STABLE")
    return INTEREST_RATE_STABLE;
  return INTEREST_RATE_VARIABLE;
}
function getATokenAddress(runtime2, poolAddress, assetAddress, chainSelectorName, testnet) {
  const network248 = getNetwork({
    chainFamily: "evm",
    chainSelectorName,
    isTestnet: testnet
  ...
  const aTokenAddr = Array.isArray(reserveData) ? reserveData[8] : reserveData.aTokenAddress;
  return aTokenAddr;
}
function encodeLendingReport(params) {
  return encodeAbiParameters(parseAbiParameters("uint8 operation, address poolAddress, address asset, uint256 amount, address walletAddress, address onBehalfOf, uint256 interestRateMode, uint16 referralCode, address aTokenAddress"), [
    params.operation,
    params.poolAddress,
    params.asset,
...
function doLending(runtime2, config) {
  ...
  const opCode = operation === "SUPPLY" ? OP_SUPPLY : operation === "WITHDRAW" ? OP_WITHDRAW : operation === "BORROW" ? OP_BORROW : OP_REPAY;
  let resolved;
  try {
  ...
  if (operation === "WITHDRAW") {
    if (!aTokenAddress || aTokenAddress === "0x0000000000000000000000000000000000000000") {
      aTokenAddress = getATokenAddress(runtime2, poolAddress, assetAddress, chainSelectorName, testnet);
    }
  }
  ...
  runtime2.log(`Aave ${operation} on ${config.chain}: asset=${assetAddress} amount=${amount} ` + `wallet=${walletAddress} onBehalfOf=${onBehalfOf}`);
  const reportPayloadBytes = reportPayloadHex.startsWith("0x") ? reportPayloadHex : `0x${reportPayloadHex}`;
  ...
  runtime2.log(`Aave ${operation} tx succeeded: ${txHash}`);
  return JSON.stringify({
    success: true,
    txHash,
    operation,
...
var onHttpTrigger = (runtime2, payload) => {
  runtime2.log("Aave lending — HTTP trigger (immediate execution)");
  const fileConfig = runtime2.config;
  let overrides = {};
  if (payload.input && payload.input.length > 0) {
    try {
      overrides = decodeJson(payload.input);
      runtime2.log(`Config overrides from payload: ${JSON.stringify(overrides)}`);
    } catch {
      runtime2.log("No parseable overrides in HTTP payload; using file config");
    }
  }
  const merged = deepMerge(fileConfig, overrides);
  return doLending(runtime2, merged);
};
