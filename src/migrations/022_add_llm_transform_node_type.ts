...
/**
 * Migration: Add LLM_TRANSFORM node type to constraints
 * 
 * This migration updates the valid_node_type constraint to include LLM_TRANSFORM
...
export const up = async (pool: Pool): Promise<void> => {
    ...
    `);
        ...
        // Add new constraint with ALL node types including LLM_TRANSFORM
        await client.query(`
      ALTER TABLE workflow_nodes
    ...
    `);
        ...
        logger.error({ error }, 'Failed to add LLM_TRANSFORM node type');
        throw error;
    } finally {
        client.release();
    }
...
export const down = async (pool: Pool): Promise<void> => {
    ...
    // Node types without LLM_TRANSFORM for rollback
    const NODE_TYPES_WITHOUT_NEW = ALL_NODE_TYPES.filter(t => t !== 'LLM_TRANSFORM');
    ...
    try {
      ...
      SELECT COUNT(*) as count FROM workflow_nodes WHERE type = 'LLM_TRANSFORM'
    `);

        if (parseInt(existingNodes.rows[0].count) > 0) {
            throw new Error('Cannot rollback: LLM_TRANSFORM nodes exist in the database. Delete them first.');
        }
    ...
    `);
        ...
        await client.query('ROLLBACK');
        logger.error({ error }, 'Failed to remove LLM_TRANSFORM node type');
        throw error;
    } finally {
        client.release();
    }
